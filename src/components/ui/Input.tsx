...
// [DOCS] Props Input custom dengan Label dan Error Message.
interface InputProps extends InputHTMLAttributes<HTMLInputElement> {
    label?: string;
    error?: string;
}

// [DOCS] Komponen Input Reusable.
// Fitur utama: Toggle Visibility Password & Error State Styling.
const Input = forwardRef<HTMLInputElement, InputProps>(
    ({ className, type, label, error, ...props }, ref) => {
        const [isVisible, setIsVisible] = useState(false);

        // [DOCS] Logic untuk toggle tipe input antara 'password' dan 'text'.
        ...
        return (
            <div className="w-full space-y-2">
                ...
                <div className="relative group">
                    <input
                        ...
                        className={cn(
                            ...
                            // [DOCS] Ubah warna border & background jadi merah jika ada error
                            error && 'border-red-300 bg-red-50 text-red-900 focus:border-red-500 focus:ring-red-500/10 placeholder:text-red-300',
                            isPasswordType && 'pr-12', // [DOCS] Beri ruang untuk icon mata
                            className
                        )}
                        {...props}
                    />

                    {/* [DOCS] Tombol mata untuk show/hide password */}
                    {isPasswordType && (
                        <button
                            type="button"
                ...
                {/* [DOCS] Tampilkan pesan error di bawah input */}
                {error && (
